...
A script is available to perform the pre-processing steps, file conversion, and peak calling in a single workflow. Additionally, individual scripts for the remaining post-processing steps are provided for flexibility and modular execution. A conda environment is available to aid in getting the code running (Conda_Env_Main_ATACSeq_JF.yml).
